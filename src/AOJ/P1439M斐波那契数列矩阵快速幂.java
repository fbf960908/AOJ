...
public class P1439M斐波那契数列矩阵快速幂 {
	...
	static final long[][] unit = { { 1, 1 }, { 1, 0 } };// ��λ����
	static final long le9 = 1000000007;

	public static long mul(long a, long b, long c) {// ���ٳ˷�
		long ans = 0, tmp = a % c;
		while (b != 0) {
			if ((b & 1) == 1) {
				if ((ans += tmp) >= c) {
	...
	public static long mul_mod(long a, long b, long c) {// ������ȡģ
		long ret = 1 % c;
		while (b != 0) {
			if ((b & 1) == 1) {
				ret = mul(ret, a, c);
	...
	public static long[][] mul_Power(long[][] a, long n) {// ���������
